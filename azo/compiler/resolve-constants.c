...
static AZONode *calculate_shift (AZONode *expr);
static AZONode *calculate_arithmetic (AZONode *expr);

static void
substitute (AZONode *expr, AZONode *old, AZONode *parent, AZONode *ref)
{
	if (ref) {
		ref->next = expr;
	} else {
...
AZONode *
azo_compiler_resolve_binary (AZONode *expr)
{
...
static AZONode *
calculate_arithmetic (AZONode *expr)
{
	AZONode *lhs, *rhs;
	AZValue a, b, c;
	unsigned int type;
	...
	if ((lhs->term.subtype == AZ_TYPE_COMPLEX_DOUBLE) || (rhs->term.subtype == AZ_TYPE_COMPLEX_DOUBLE) ||
		...
		if (expr->term.subtype == AZO_TERM_ARITHMETIC_PLUS) {
			...
			c.int64_v = a.int64_v % b.int64_v;
		} else if (expr->term.subtype == AZO_TERM_ARITHMETIC_AND) {
			c.int64_v = a.int64_v & b.int64_v;
		} else if (expr->term.subtype == AZO_TERM_ARITHMETIC_OR) {
			c.int64_v = a.int64_v | b.int64_v;
		} else if (expr->term.subtype == AZO_TERM_ARITHMETIC_CARET) {
			c.int64_v = a.int64_v ^ b.int64_v;
		} else {
			fprintf (stderr, "calculate: Invalid arithmetic operation for int64\n");
			expr->term.type = AZO_TERM_INVALID;
